...
axios.defaults.baseURL = process.env.VUE_APP_URL // the prefix of the URL
...
let failedQueue = [];
const unwantedAccess = ['auth/token/', 'auth/signup/merchant/'];
...
axios.interceptors.response.use(function (response) {
  ...
  if (error.response.status === 401 && originalRequest.url === 'auth/refresh/') {
    if (router.history._startLocation != "/login/merchant") {
      localStorage.setItem('loginContinue', router.history._startLocation)
    }
    store.dispatch('auth/logout');
  }
